#!/bin/bash
#SBATCH --job-name=qwen1.5b-study
...
#SBATCH --exclusive
#SBATCH --output=logs/qwen1.5b-%j-%x.out
#SBATCH --error=logs/qwen1.5b-%j-%x.err

# =============================================================================
# Qwen2.5-1.5B Training-Generation Mismatch Study
#
# Dense model, single-node, TP=1.
# Runs three experiment modes to measure gen_kl_error:
#   vllm             — vLLM baseline (expected: non-zero gen_kl_error)
#   m-inf            — Megatron-Inference without FA3 (expected: ~6e-4)
#   m-inf-fa3        — Megatron-Inference + Flash Attention 3 (target: ~0)
#   batch-invariant  — Megatron-Inference + FA3 + batch_invariant_mode (reference)
#
...
#
# Override partition/time:
#   sbatch -p interactive --time=1:00:00 --export=MODE=m-inf run_qwen1.5b_study.sh
#
# Override max steps:
#   sbatch --export=MODE=m-inf,MAX_STEPS=100 run_qwen1.5b_study.sh
# =============================================================================
...
echo "=============================================="
echo "Qwen2.5-1.5B  gen_kl_error study"
echo "  Mode:          ${MODE}"
...
case "$MODE" in
    vllm)
        CMD="${BASE_CMD} \
            ...
            logger.wandb.name=qwen-1.5b-vllm"
        ;;
    m-inf)
        CMD="${BASE_CMD} ${MINF_FLAGS} \
            logger.wandb.name=qwen-1.5b-m-inf"
        ;;
    m-inf-fa3)
        export NRL_INSTALL_FA3=1
        CMD="${BASE_CMD} ${MINF_FLAGS} \
            logger.wandb.name=qwen-1.5b-m-inf-fa3"
        ;;
    batch-invariant)
        export NRL_INSTALL_FA3=1
        CMD="${BASE_CMD} ${MINF_FLAGS} \
            ...
            logger.wandb.name=qwen-1.5b-m-inf-batch-invariant"
        ;;
esac
